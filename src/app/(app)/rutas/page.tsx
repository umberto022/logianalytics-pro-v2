...
const RouteMap = dynamic(() => import("@/components/map/RouteMap"), {
  ssr: false,
  loading: () => (
    <div className="h-full flex items-center justify-center bg-slate-100 rounded-2xl dark:bg-slate-800">
  ...
  if (canViewAnalytics && routes.length === 0 && tab === "analytics") {
    return (
      <div>
        ...
        <div className="bg-white rounded-2xl border border-slate-100 p-12 text-center shadow-sm dark:bg-slate-800 dark:border-slate-700">
          <MapPin size={52} className="text-slate-200 mx-auto mb-4 dark:text-slate-600" />
          <h2 className="text-xl font-semibold text-slate-700 mb-2 dark:text-slate-200">Sin datos de rutas</h2>
  ...
  return (
    <div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md p-6 dark:bg-slate-800">
            <div className="flex items-center justify-between mb-5">
              <h2 className="font-bold text-slate-800 dark:text-slate-100">{editing ? "Editar ruta" : "Nueva ruta"}</h2>
              <button onClick={() => { setShowForm(false); setEditing(null); }} className="p-1.5 rounded-xl hover:bg-slate-100 text-slate-400 transition dark:hover:bg-slate-700"><X size={18} /></button>
            </div>
            <div className="space-y-3">
              ...
              <div>
                ...
                <textarea value={form.description} onChange={(e) => setForm((p) => ({ ...p, description: e.target.value }))}
                  ...
                  className="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-brand-500 resize-none dark:border-slate-700 dark:bg-slate-800 dark:text-slate-100 dark:placeholder-slate-400" />
              </div>
              <label className="flex items-center gap-2 cursor-pointer">
                <input type="checkbox" checked={form.active} onChange={(e) => setForm((p) => ({ ...p, active: e.target.checked }))}
                  className="w-4 h-4 rounded accent-brand-600" />
                <span className="text-sm font-medium text-slate-700 dark:text-slate-300">Ruta activa</span>
              </label>
            </div>
            <div className="flex gap-3 mt-6">
              <button onClick={() => { setShowForm(false); setEditing(null); }}
                className="flex-1 py-2.5 border border-slate-200 text-slate-700 text-sm font-semibold rounded-lg hover:bg-slate-50 transition dark:border-slate-700 dark:text-slate-300 dark:hover:bg-slate-700">Cancelar</button>
      ...
      <div className="flex gap-1 bg-slate-100 rounded-xl p-1 mb-6 w-fit dark:bg-slate-800">
        {(canViewAnalytics
          ...
          <button key={key} onClick={() => setTab(key as Tab)}
            className={`px-4 py-2 text-sm font-medium rounded-lg transition whitespace-nowrap ${tab === key ? "bg-white shadow-sm text-brand-600 dark:bg-slate-700 dark:text-brand-400" : "text-slate-500 hover:text-slate-700 dark:text-slate-400 dark:hover:text-slate-200"}`}>
            {label}
          </button>
        ))}
      </div>
        ...
        <div>
          {managed.length === 0 ? (
            <div className="bg-white rounded-2xl border border-slate-100 p-16 text-center shadow-sm dark:bg-slate-800 dark:border-slate-700">
              ...
              <p className="text-slate-500 font-medium dark:text-slate-400">No tienes rutas creadas</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
        ...
        {/* Ranking list */}
        <div className="bg-white rounded-2xl border border-slate-100 shadow-sm flex flex-col overflow-hidden dark:bg-slate-800 dark:border-slate-700">
          <div className="px-4 py-3 border-b border-slate-100 flex-shrink-0 dark:border-slate-700">
            <h3 className="font-semibold text-slate-700 text-sm dark:text-slate-200">Ranking por ganancia</h3>
            ...
            {routes.map((r, i) => {
              ...
              return (
                <button
                  ...
                  className={`w-full text-left px-4 py-3 border-b border-slate-50 hover:bg-slate-50 transition-all dark:border-slate-700/50 dark:hover:bg-slate-700/50 ${active ? "bg-brand-50 border-l-4 border-l-brand-500 dark:bg-brand-500/10" : ""}`}
                >
                  <div className="flex items-center justify-between mb-1.5">
                    <div className="flex items-center gap-2 min-w-0">
                      <span className="text-xs font-bold text-slate-300 w-5 flex-shrink-0 dark:text-slate-600">#{i + 1}</span>
                      <span className="text-sm font-semibold text-slate-700 truncate dark:text-slate-200">{r.route}</span>
                    </div>
                    <span className={`text-xs font-bold px-2 py-0.5 rounded-full flex-shrink-0 ml-1 ${
                      r.marginPct >= 20 ? "bg-emerald-100 text-emerald-700 dark:bg-emerald-500/15 dark:text-emerald-300"
                      : r.marginPct >= 10 ? "bg-amber-100 text-amber-700 dark:bg-amber-500/15 dark:text-amber-300"
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs text-slate-500 whitespace-nowrap dark:text-slate-400">{fmtCurrency(r.profit)}</span>
                  </div>
                </button>
              );
            })}
      ...
      {selectedRoute && (
        <div className="bg-white rounded-2xl border border-brand-200 p-5 shadow-sm mb-6 animate-slide-up dark:bg-slate-800 dark:border-brand-500/30">
          <div className="flex items-center justify-between mb-4">
            <h3 className="font-bold text-slate-800 flex items-center gap-2 dark:text-slate-100">
              <MapPin size={16} className="text-brand-500" />
              {selectedRoute.route}
            </h3>
            <button onClick={() => setSelected(null)} className="text-xs text-slate-400 hover:text-slate-600 px-2 py-1 rounded hover:bg-slate-100 transition dark:hover:text-slate-200 dark:hover:bg-slate-700">
              ✕ cerrar
            </button>
          </div>
      ...
      <div className="flex flex-col sm:flex-row gap-3 mb-6">
        {best && (
          <div className="flex-1 bg-emerald-50 border border-emerald-200 rounded-xl px-4 py-3 text-sm text-emerald-800 dark:bg-emerald-500/15 dark:border-emerald-500/30 dark:text-emerald-300">
        ...
        {worst && worst !== best && worst.marginPct < 10 && (
          <div className="flex-1 bg-amber-50 border border-amber-200 rounded-xl px-4 py-3 text-sm text-amber-800 dark:bg-amber-500/15 dark:border-amber-500/30 dark:text-amber-300">
      ...
      <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden dark:bg-slate-800 dark:border-slate-700">
        <div className="px-5 py-4 border-b border-slate-100 dark:border-slate-700">
          <h3 className="font-semibold text-slate-700 dark:text-slate-200">Tabla completa</h3>
        </div>
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {routes.map((r, i) => (
                <tr
                  key={r.route}
                  className={`border-t border-slate-50 cursor-pointer transition-colors dark:border-slate-700/50 ${selected === r.route ? "bg-brand-50 dark:bg-brand-500/10" : "hover:bg-slate-50 dark:hover:bg-slate-700/50"}`}
                  onClick={() => setSelected(selected === r.route ? null : r.route)}
                >
                  <td className="py-2.5 px-4 text-slate-300 font-bold dark:text-slate-600">#{i + 1}</td>
                  <td className="py-2.5 px-4 font-semibold text-slate-800 dark:text-slate-100">{r.route}</td>
                  ...
                  <td className="py-2.5 px-4 text-indigo-600 font-medium dark:text-indigo-400">{fmtCurrency(r.revenue)}</td>
                  <td className="py-2.5 px-4 text-red-500 dark:text-red-400">{fmtCurrency(r.cost)}</td>
                  <td className="py-2.5 px-4 text-emerald-600 font-semibold dark:text-emerald-400">{fmtCurrency(r.profit)}</td>
