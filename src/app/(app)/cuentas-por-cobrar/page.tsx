  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        {[
          { label: "Por cobrar",       value: fmtCurrency(totalPending),   icon: CreditCard,   color: "text-indigo-600 bg-indigo-50" },
          { label: "Vencidas",         value: fmtCurrency(totalOverdue),   icon: AlertTriangle, color: "text-red-600 bg-red-50" },
          { label: "Vencen en 7 días", value: upcoming.length,             icon: Clock,        color: "text-amber-600 bg-amber-50" },
          { label: "Cobradas",         value: fmtCurrency(paid.reduce((s, v) => s + v.totalRevenue, 0)), icon: CheckCircle2, color: "text-emerald-600 bg-emerald-50" },
      ...
      <div className="flex gap-2 mb-4 flex-wrap">
        {([
          ...
          <button key={f.key} onClick={() => setFilter(f.key)}
            className={`px-3 py-1.5 text-xs font-semibold rounded-lg border transition ${filter === f.key ? "bg-brand-600 text-white border-brand-600" : "border-slate-200 text-slate-600 hover:bg-slate-50"}`}>
            {f.label}
          </button>
        ))}
      </div>
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {pagination.paged.map((s) => {
                  ...
                  return (
                    <tr key={s.id} className={`border-t border-slate-50 ${isOverdue ? "bg-red-50/40" : isDue7 ? "bg-amber-50/40" : ""}`}>
                      <td className="py-3 px-4 text-slate-500 whitespace-nowrap">{fmtDate(s.saleDate)}</td>
                      <td className="py-3 px-4 font-medium truncate max-w-[130px]">{s.client || "—"}</td>
                      <td className="py-3 px-4 text-slate-600 truncate max-w-[130px]">{s.productName}</td>
                      <td className="py-3 px-4 font-semibold text-slate-900">{fmtCurrency(s.totalRevenue)}</td>
                      <td className={`py-3 px-4 whitespace-nowrap text-xs font-semibold ${isOverdue ? "text-red-600" : isDue7 ? "text-amber-600" : "text-slate-500"}`}>
                        {dueDate ? fmtDate(s.dueDate!) : "—"}{isOverdue ? " ⚠️" : ""}
                      </td>
                      <td className="py-3 px-4">
                        {s.paymentStatus === "pagado" ? (
                          <span className="text-xs font-semibold text-emerald-700 bg-emerald-50 px-2 py-0.5 rounded-full">Cobrado</span>
                        ) : (
                          <span className={`text-xs font-semibold px-2 py-0.5 rounded-full ${isOverdue ? "text-red-700 bg-red-100" : "text-amber-700 bg-amber-50"}`}>
