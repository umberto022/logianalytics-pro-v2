...
const NAV = [
  { href: "/dashboard",     label: "Dashboard",     icon: LayoutDashboard, badge: null,       pro: false },
  { href: "/ventas",        label: "Ventas",        icon: ShoppingCart,    badge: null,       pro: false },
  { href: "/clientes",      label: "Clientes",      icon: Users,           badge: null,       pro: false },
  { href: "/rutas",         label: "Rutas",         icon: MapPin,          badge: null,       pro: true  },
  { href: "/rentabilidad",  label: "Rentabilidad",  icon: TrendingUp,      badge: null,       pro: true  },
  { href: "/compras",       label: "Compras",       icon: ClipboardList,   badge: null,       pro: false },
  { href: "/inventario",    label: "Inventario",    icon: Package,         badge: "stock" as const, pro: false },
  { href: "/configuracion", label: "Configuración", icon: Settings,        badge: null,       pro: false },
];
  ...
  return (
    <aside className="hidden lg:flex fixed inset-y-0 left-0 w-[var(--sidebar-width)] bg-sidebar text-slate-200 flex-col z-20">
      ...
      <nav className="flex-1 px-3 py-4 space-y-0.5 overflow-y-auto">
        {NAV.map(({ href, label, icon: Icon, badge, pro }) => {
          ...
          const inner = (
            <>
              ...
              {locked && <Lock size={12} className="text-slate-600 flex-shrink-0" />}
              {showBadge && (
