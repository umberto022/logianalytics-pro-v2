...
const TYPES: { value: FeedbackType; label: string; icon: React.ReactNode; color: string }[] = [
  { value: "bug",        label: "Bug",        icon: <Bug size={14} />,         color: "bg-red-50 text-red-600 border-red-200 hover:bg-red-100"      },
  { value: "sugerencia", label: "Sugerencia", icon: <Lightbulb size={14} />,   color: "bg-amber-50 text-amber-600 border-amber-200 hover:bg-amber-100" },
  { value: "pregunta",   label: "Pregunta",   icon: <HelpCircle size={14} />,  color: "bg-blue-50 text-blue-600 border-blue-200 hover:bg-blue-100"   },
];
  ...
  return (
    <>
      ...
      <button
        ...
        className="fixed bottom-20 left-4 lg:bottom-6 lg:left-6 z-40 w-11 h-11 bg-white border border-slate-200 shadow-md hover:shadow-lg text-slate-500 hover:text-indigo-600 hover:border-indigo-300 rounded-xl transition-all flex items-center justify-center"
      >
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm">
            <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100">
              <div className="flex items-center gap-2">
                <MessageSquarePlus size={17} className="text-indigo-600" />
                <span className="font-semibold text-slate-800 text-sm">Enviar feedback</span>
              </div>
                  ...
                  {TYPES.map((t) => (
                    <button
                      ...
                      className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg border text-xs font-medium transition ${
                        type === t.value ? t.color + " border-current" : "bg-white text-slate-500 border-slate-200 hover:bg-slate-50"
                      }`}
                    >
                      {t.icon} {t.label}
                    </button>
